                ...
                if not visited_tld in third_party_tld:
                    ...
                    if temp_orga is not None:
                        ...
                        if not temp_orga in third_party_orgas:
                            third_party_orgas.append(str(temp_orga))
                    else:
                        ...
                        if not third_party_tld in third_party_orgas:
                            third_party_orgas.append(str(third_party_tld))
                    if rules.should_block(url) is True:
                        ...
                        if temp_orga is not None:
                            ...
                            if not temp_orga in advertising_orgas:
                                advertising_orgas.append(str(temp_orga))
                        else:
                            if not third_party_tld in all_advertising_orgas:
                                all_advertising_orgas.append(str(third_party_tld))
                            if not third_party_tld in advertising_orgas:
                                advertising_orgas.append(str(third_party_tld))

            else:
                raise ValueError('http is not in url!', url)

        resObject["third_party_orgas"] = third_party_orgas
        resObject["advertising_orgas"] = advertising_orgas
    ...
    for third_party_site in all_third_party_orgas:
        ...
        for resObject in result:
            if resObject['index'] == display_index and resObject['success'] is True:
                if third_party_site in resObject['third_party_orgas']:
                    tp_occurences += 1
                if third_party_site in resObject['advertising_orgas']:
        ...
        for resObject in result:
            if resObject['index'] <= display_index:
                ...
                if site_was_successful is True and site_has_ad is False:
                    ...
                    if ad_site in resObject['advertising_orgas']:
                        advertiser_occurences += 1
                        site_has_ad = True
        advertiser_percentages_array.append(getPercentage(advertiser_occurences, len(all_crawled_sites)))
...
df = df.head(30)
df.to_csv('tables/cumulated_top_advertiser_orgas_2.csv', sep='\t', encoding='utf-8', index=False)
