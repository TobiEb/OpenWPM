...
for advertiser_site in all_advertising_sites:
    advertiser_occurences = 0
    for resObject in result:
        if resObject['index'] == display_index:
            if advertiser_site in resObject['advertising_sites']:
...
plt.bar(all_advertising_sites, advertiser_percentages_array, color="red")
